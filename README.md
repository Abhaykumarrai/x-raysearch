...
| `VITE_APOLLO_WEBHOOK_URL` | _(optional)_ Apollo phone reveal | — | Apollo requires a **public `webhook_url`** when `reveal_phone_number` is true (they POST the number when ready). On live HTTPS deployments, this app auto-falls back to `https://<your-domain>/api/apollo-webhook` when this env var is unset. Set this var only if you want a custom endpoint. |
